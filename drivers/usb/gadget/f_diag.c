...
static const char cdc_interface[] = "-au URBANO PROGRESSO High Speed Serial Port";
		...
		unsigned intf, unsigned alt)
{
	...
	dev->in_desc = ep_choose(cdev->gadget,
			(struct usb_endpoint_descriptor *)f->hs_descriptors[3],
			(struct usb_endpoint_descriptor *)f->descriptors[3]);
	dev->out_desc = ep_choose(cdev->gadget,
			(struct usb_endpoint_descriptor *)f->hs_descriptors[4],
			(struct usb_endpoint_descriptor *)f->descriptors[4]);
