...
extern void dhd_set_dtim_value(dhd_pub_t *dhd);
#endif
/* 2012-03-15 Add End */
	...
	struct beacon_parameters *info)
{
	...
	/* 2012-04-22 Debug Start */
	printk(KERN_INFO "%s: interval (%d) dtim_period (%d) head_len (%d) "
		"tail_len (%d)\n", __func__, info->interval,
		info->dtim_period, info->head_len, info->tail_len);
	/* 2012-04-22 Debug End */
	if (wl->p2p_net == dev) {
		dev = wl_to_prmry_ndev(wl);
	}
	...
	if (p2p_is_on(wl) &&
		...
		/* 2012-04-22 Debug Start */
		printk(KERN_INFO "%s: bssidx=%d p2p->bss_idx[type].bssid=%d\n",
			__func__, bssidx,
...
/* 2012-03-15 Add Start */
#ifdef CONFIG_ARCH_MSM7X30
	{
		...
		if (dhd && dhd->up) {
			dhd->beacon_interval = beacon_interval;
			dhd_set_dtim_value(dhd);
		}
	}
#endif
/* 2012-03-15 Add End */
	...
	/* 2012-03-15 Debug Start */
	printk(KERN_INFO "%s: beacon_interval=%d, dtim_period=%d\n",
		__func__, beacon_interval, dtim_period);
	/* 2012-03-15 Debug End */
...
void wl_cfg80211_detach(void *para)
{
	...
	WL_TRACE(("In\n"));
	/* 2012-04-21 Debug Start */
	printk(KERN_INFO "%s: Enter\n", __func__);
	/* 2012-04-21 Debug End */
...
static s32 __wl_cfg80211_down(struct wl_priv *wl)
{
	...
	if (p2p_is_on(wl)) {
		p2p_on(wl) = false;
		printk(KERN_INFO "%s p2p mode off\n", __func__);
	}
	/* 2012-04-21 Debug End */
	return err;
}
