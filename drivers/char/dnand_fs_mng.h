...
#define DNAND_FS_PARTITION_SIZE           (1024*1024*4)
...
#ifdef IMAGE_MODEM_PROC
    #define DNAND_FS_BLK_CLID_NUM             ( (DNAND_FS_CLID_NUM*sizeof(uint32)+DNAND_FS_BLK_SIZE-1)/DNAND_FS_BLK_SIZE )
    #define DNAND_FS_BLK_FAT_NUM              ( (DNAND_FS_BLK_NUM*sizeof(uint16)+DNAND_FS_BLK_SIZE-1)/DNAND_FS_BLK_SIZE )
#else
...
#define DNAND_FS_BLK_EOF                  (0x7FFF)
    ...
    {
        uint8    ckcd[DNAND_FS_CKCD_SIZE];
        uint8    dmya[DNAND_FS_BLK_SIZE-DNAND_FS_CKCD_SIZE];
        uint32   clid[(DNAND_FS_BLK_CLID_NUM*DNAND_FS_BLK_SIZE)/sizeof(uint32)];
        uint16   fat[(DNAND_FS_BLK_FAT_NUM*DNAND_FS_BLK_SIZE)/sizeof(uint16)];
    }dnand_fs_flmng;

    typedef struct dnand_fs_memng_st
    {
        uint32   clid[DNAND_FS_CLID_NUM];
        uint16   fat[DNAND_FS_BLK_NUM];
    }dnand_fs_memng;
#else
    typedef struct dnand_fs_flmng_st
    {
